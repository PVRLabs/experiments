...
```bash
java -Xmx64m -XX:+UseSerialGC -Xss256k -jar stars.jar
```
...
```bash
nohup java -Xmx64m -XX:+UseSerialGC -Xss256k -jar stars.jar > stars.log 2>&1 &
```
...
ExecStart=/usr/bin/java -Xmx64m -XX:+UseSerialGC -Xss256k -jar /opt/stars-app/stars.jar
Restart=on-failure
...
Do not unzip and try to `mvn-lite` on that box. There is nothing to compile — only `java -jar stars.jar`.
...
`run.sh` and `mvn-lite spring-boot:run` use `-Xmx64m -XX:+UseSerialGC -Xss256k`.
...
| `/actuator/prometheus` | Prometheus exposition from the same application server |
...
Only `health`, `info`, `metrics`, and `prometheus` are exposed. Heap dump, env, and beans stay off.
...
Dashboard: [http://127.0.0.1:9090](http://127.0.0.1:9090)
...
export GITHUB_TOKEN=ghp_...   # optional; raises the unauthenticated 60 req/hour cap
```
